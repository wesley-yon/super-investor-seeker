...
- `scripts/build_pages_artifact.py`, `.github/workflows/deploy-pages.yml`, and `.github/workflows/update-data.yml`: connect the live UI and any later explicitly authorized publication schedule to the existing manual materializer, validation, visual/accessibility checks, exact snapshot publication, artifact audits, and deploy gates without widening public exposure.
...
Phase 5 stop: live normalized SEC payloads replace the browser fixture adapter, the price/timeline state is honest and basis-correct, existing holders remain unregressed, accessibility/performance/visual checks pass, and operations/recovery are documented.
