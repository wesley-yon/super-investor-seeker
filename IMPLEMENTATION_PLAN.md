...
This document originated as the Phase 0 repository-discovery deliverable. The plan remains the design record; factual stack and verification notes below have been updated after the authorized Phase 1–5 implementation so they do not misdescribe the current checkout. The illustrative appendices remain non-authoritative.

## Implementation status

Phases 1–5 are **completed as-built history** in this checkout: Phase 1 delivered the local-only, default-off illustrative fixture UI and rendered-browser coverage; Phase 2 delivered the validated Section 16 contract/parser foundations; Phase 3 delivered ingestion, backfill, snapshot, and workflow integration; Phase 4 delivered bounded canonical static insider metrics, privacy-screened publication, a manual/default-off production materializer, validation, and deterministic Pages packaging; and Phase 5 connected the live stock subroutes to those validated same-origin payloads, added digest-bound filing details, preserved the local fixture preview, shipped the honest transaction-only price fallback, and completed final UI hardening. Phase 1 is local-only and default-off. The detailed sections below record delivered scope rather than authorize new work. A future daily-price provider, public owner index, automated materialization, or incompatible contract remains separately reviewable work.
...
| Router | Hand-written hash routing. Supported routes are `#fund/<CIK>`, `#stock/<stockId>`, and the closed `insiders` / `reporting-insiders` stock subviews. The local fixture preview is a query-flagged adapter for the same subviews, not a separate route. Delegated stock actions canonicalize an ID and resolve it against the loaded search index before routing. `history.pushState` plus `hashchange`/`popstate` is used. | `index.html`: `wireUrlRouting`, `routeFromHash`, `setUrl`, `resolveDelegatedStockId` |
...
| Visual/accessibility testing | Pinned Playwright exercises fixture and live routes, five viewports, keyboard behavior, focus, responsive layout, URL-backed pagination, privacy-safe filing drawers, and deterministic screenshots. Static semantic and exact generator-bound fixture tests remain as independent layers. | `playwright.config.js`; `tests/visual/`; `tests/test_insider_phase1.py`; `tests/test_insider_phase5.py` |
...
`index.html` parses `#fund/<id>` and `#stock/<id>`, including the closed live
`insiders` and `reporting-insiders` stock subviews. Stock IDs are canonical
public security identities, not necessarily ticker strings. Delegated controls
canonicalize their `data-stock-id` and require a resolved entry from the loaded
search index before they can invoke stock loading. `stockFilePath` maps an
already-resolved equity ID to `data/stocks/<safe-id>.json` and a non-equity ID
to `data/stocks/<safe-id>__<INSTRUMENT_TYPE>.json`. The live insider adapter
maps that same identity through the bounded public file-stem grammar and loads
`data/insiders/public/securities/<file-stem>.json` from the same origin.

The implementation preserves the holder route and uses view suffixes rather
than inventing `/security/:ticker` paths:
...
Insider filter and client-page state use the real query string because `setUrl`
preserves `location.search`, for example
`?range=1y&transactionScope=ps&page=2#stock/03770N101/insiders`. A filing drawer
uses the validated accession in a `filing` query parameter. The local-only,
default-off `?insiderPreview=fixture` flag may substitute the illustrative APGE
fixture on loopback hosts only; deployed hosts ignore it, and production fetch
or contract failures never fall through to it. The exact hash/query grammar is
covered by semantic and rendered tests.
...
There is no row-query database or cursor-capable server API. The insider logical model is immutable, normalized, versioned JSON in private snapshot storage plus bounded per-security public projections. Canonical metrics are calculated in Python with `decimal.Decimal` and serialized as exact decimal strings; browser code formats and filters validated values but does not become the financial source of truth. `null` remains unknown throughout Python, JSON, and UI.
...
- CUSIP/security ID is canonical for the present 13F product; ticker and issuer are descriptive and may be absent or remapped. The insider model uses issuer CIK as its durable private Section 16 issuer key and maps each as-filed security class to the existing stock ID without merging distinct classes.
- Filing accession number is the immutable public identifier for insider filing detail. Reporting-owner CIK and the sorted-CIK owner-group hash exist only in private ingestion/state for ownership identity and joint-filer counting; public projections omit both and never recreate a cross-filing owner key.
...
Reusable conventions are the shared SEC session/rate limiter, accession checkpointing/quarantine, atomic JSON writes, immutable snapshot publication, exact artifact audit, and existing workflow gates. Phases 2–3 added ownership-form discovery/parsing, quarterly insider dataset backfill, immutable raw ownership XML retention, parser-version storage/reparse tooling, insider amendment matching, owner/security normalization, and insider-specific telemetry. That private immutable Section 16 source store is distinct from the 13F parser and preserves ownership source evidence for deterministic reparse.
...
## As-built history mapped to PRD Phases 1-5

The mapping below follows the five phases in `CODEX_START_HERE.md`. The longer PRD later separates frontend integration and hardening into additional phases; here those tasks are folded into Phase 5 as the user requested. All five phases are recorded as-built history; any post-Phase-5 extension requires a new review boundary.
...
- `tests/test_frontend_semantics.py` and `tests/test_insider_phase1.py` cover routes, navigation semantics, URL filters, null rendering, drawer focus, source-link separation, the P/S default, and the production-host fixture boundary. In that historical phase, Reporting Insiders is implemented fixture scope. Phase 5 later replaced the production route adapter while retaining the local preview.
...
- `insider_publication.py` is the sole private-to-public boundary. It revalidates normalized records and issuer state, resolves effective amendments, and emits bounded, versioned per-security page payloads plus per-accession detail payloads. Detail owners expose only each person's name as filed, a strictly screened company-related title, and normalized role labels; approved name fields reject standalone 10-digit CIK-shaped tokens, 64-hex private correlator-shaped tokens, and contact/address/path text. Addresses, owner CIKs, cross-filing owner identifiers or aggregation slots, signatures, remarks, full footnote text, ownership narratives, raw security-title text, parser diagnostics, raw source, and private paths/hashes are excluded. Owner-sensitive counts, rankings, holdings, and Rule 10b5-1 aggregates are fully recomputed from canonical private owner-bound rows immediately before serialization. Public-tree validation exactly reconstructs identity-independent metrics from filing details and strictly validates the shape, bounds, arithmetic, ordering, and public-owner plausibility of identity-derived fields without recreating or publishing a cross-filing owner key.
- `insider_storage.py` adds the exact private `publication-policy-v1` contract. The policy is a reviewed public-publishing allowlist whose issuer set may be a strict subset of ingestion-approved issuers; ingestion approval alone cannot make an issuer public. Each policy issuer carries the complete exact security-class mapping required by the static publication contract.
- `scripts/publish_insider_activity.py` is the bounded offline production adapter. It requires one exact completed incremental, backfill, or reparse checkpoint and explicit canonical UTC freshness timestamps. Incremental v1 carries issuer scope only in queue entries, so an empty completed incremental checkpoint is valid for ingestion but is rejected as unbound for publication. The adapter opens each canonical normalized artifact by the exact SHA-256 recorded in issuer state without invoking the filing-index or ownership-XML parsers; re-derives and exactly reconciles every policy issuer's state and normalized hashes; requires complete security mappings and zero unmapped public rows; enforces aggregate ceilings of 15,000 normalized filings and 250 MB of canonical normalized input; builds the complete policy corpus in memory; and invokes the journaled public writer exactly once. It has a fixed repository/output boundary, performs no network request, backfill, or reparse itself, and fails before mutation on missing, stale, incomplete, unapproved, noncanonical, or over-budget state.
- `.github/workflows/update-data.yml` exposes materialization only as a manual `workflow_dispatch` boolean that defaults false. Merge, push, and scheduled events cannot select it. When explicitly requested, it runs after private checkpoint validation and before public validation, the full regression suite, snapshot publication, and Pages selection; exact `as_of` and latest-sync inputs are mandatory.
- Static per-security payloads are generated with `range=all` and `transactionScope=all` so the complete bounded canonical dataset is available for Phase 5 client filtering; Phase 5's live UI defaults its view to one year and P/S without deleting older or non-P/S rows from the static contract.
...
Phase 4 stop: the static contract's summary, chart events, transactions, sidebar, filing references, and quality/freshness fields reconcile to the same normalized effective rows in deterministic fixture-backed tests; financial values remain exact strings, unknown states are explicit, output is bounded, and the publication boundary is privacy-audited. Publication and artifact failure paths are fail-closed: authenticated transactions recover deterministically, while identity uncertainty preserves evidence instead of guessing at cleanup. Production materialization from a restored private snapshot is available only through the explicit manual/default-off policy gate; merge, push, and scheduled events do not generate a public insider tree. At that stop boundary, UI replacement and the price decision remained Phase 5, while any authorization to automate public materialization remained separate. Phase 5 later completed the live UI and transaction-only decision without automating materialization. No real private corpus was inspected and no public artifact was deployed during Phase 4 implementation.

### Phase 5 - completed as-built history: live integration and hardening

- `index.html` keeps the local-only, default-off APGE fixture preview but makes the `insiders` and `reporting-insiders` stock subroutes live by default. The adapter derives one canonical same-origin security path from the resolved stock identity, enforces finite response bytes and fatal UTF-8 decoding, and validates the exact contract-1 closed shape, identity, decimal/date fields, filing references, static pagination, summaries, chart events, and sidebar reconciliation before rendering. A 404 is an explicit empty state; every other fetch or validation failure is generic and cannot fall back to fixture data.
- Filing detail is on demand and fail-closed. The browser accepts only the one current validated security payload, one matching accession reference, its canonical relative path, declared size, and SHA-256 digest. It then validates the closed public filing contract and renders only privacy-screened names as filed, company relationship/title, normalized roles, transactions, holdings, and exact SEC links. Addresses, owner CIKs, stable owner identifiers, private correlators, raw narratives, source content, and private provenance remain unavailable to the browser.
- Live views default to a one-year P/S presentation while retaining the complete canonical `range=all` / `transactionScope=all` payload for client filters. Filter, sort, page, and filing state are URL-backed. The transaction table renders no more than the validated 100-row page size; summary cards, rail, and transaction timeline continue to use the complete filtered row set. The holder page retains its compact published 90-day cross-link.
- No daily market-price provider is approved. The implemented Phase 5 decision is the explicit transaction-only fallback: no daily line is drawn, a missing transaction price remains unknown, and transaction markers use only prices reported in that SEC transaction. There are no browser-to-provider calls, provider keys, split inference, currency inference, or basis-mismatched prices. Adding daily prices remains a separately approved Python/workflow and contract-extension project.
- `tests/test_insider_phase5.py` binds checked-in simple and complex public fixtures byte-for-byte to the current Phase 4 publication generator and tests contract/identity/privacy/size/digest failures. The pinned Playwright suite covers live loading, 404/error isolation, exact formatting, URL-backed pagination, reporting-owner grouping, holder cross-linking, joint owners, null prices and values, weighted-average and indirect transactions, derivative holdings, filing privacy, keyboard/drawer behavior, and responsive layout alongside the five deterministic fixture viewports. Existing parser, metrics, publication, validation, artifact, workflow, and full regressions remain the authoritative multi-issuer/amendment/durability gates.
- `scripts/build_pages_artifact.py` and both data/Pages workflows need no wider public or scheduling authority for the UI: they already validate and package the exact optional insider topology. Public generation remains the existing manual, explicit, default-off materializer; merge, push, scheduled workflows, and browser reads cannot activate it.
- `README.md` and `ARCHITECTURE.md` record the live adapter, empty/error behavior, digest-bound details, privacy surface, client pagination, honest price boundary, and unchanged operations/recovery boundary.

Phase 5 stop: validated same-origin public SEC projections replace the production browser fixture adapter; the local fixture remains isolated; the price/timeline state is explicitly transaction-only and basis-honest; holders remain unregressed; accessibility, performance, responsive, privacy, and full regression checks pass; and no private corpus, owner index, new dependency, materialization run, commit, push, merge, or deployment is implied by the implementation itself.

## Possible dependencies, with reasons

No post-Phase-5 dependency should be added until its extension is explicitly authorized and a repository-native alternative has been evaluated.
...
2. **Native SVG remains the chart boundary.** The current site already renders SVG/CSS mini charts, and Phase 5's transaction-only timeline adds no runtime chart library. If a later approved price provider requires a daily line, extend the vanilla SVG approach first; add a chart library only if keyboard event selection, responsive tooltips, and performance cannot be met. Any candidate needs a pinned version, local/allowlisted delivery, bundle/license review, and custom marker support.
...
## Post-Phase-5 decision boundaries

Phase 5 deliberately closes the live static UI without silently authorizing the
unrelated data-source or privacy expansions below.

1. The search index still has no insider owner records. An owner-search feature requires a separately approved bounded public owner index plus privacy and size review; the current publication has no global owner index or cross-filing owner identifier.
2. No daily price provider or split/currency contract is approved. The shipped transaction-only timeline is the final Phase 5 fallback, not a placeholder that permits ad hoc browser fetching. A later provider requires licensing, deterministic server-side ingestion, historical identity/basis rules, a versioned contract, and separate authorization.
3. Static pagination embeds every canonical group in a bounded per-security payload and renders validated client pages of at most 100 rows. If measured live scale exceeds an enforced row or byte ceiling, a future design must add deterministic generated shards rather than truncate or raise limits without evidence.
4. Insider canonical financial values remain Python `Decimal` strings. Browser formatting and exact client aggregation do not rewrite existing 13F JavaScript-number or float arithmetic.
5. Market capitalization, sector, shares outstanding, and ownership-percentage denominators remain unavailable when no sourced, dated contract exists. They stay omitted/null rather than being inferred from mockup values.

## Historical Phase 0 exit

Repository discovery established the original plan without replacing the stack. Phase 1 completed a default-off vanilla-JS illustrative fixture UI, Phases 2 and 3 completed the contract/parser and ingestion foundations, Phase 4 completed the bounded canonical static publication layer plus its manual/default-off production materializer, and Phase 5 completed the validated live static UI with digest-bound detail, transaction-only price semantics, and final hardening. Further work begins only at a new, explicitly authorized boundary.
