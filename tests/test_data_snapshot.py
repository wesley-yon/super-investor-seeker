...
class DataSnapshotTests(unittest.TestCase):
    ...
    def test_pack_excludes_rolled_back_insider_subtree(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            source = self.make_source(root)
            clean = self.pack(source, root / "clean")
            legacy_state = source / "data/insiders/private/state"
            legacy_state.mkdir(parents=True)
            (legacy_state / "approved-issuers-v1.json").write_text(
                '{"contract_version":1,"issuer_ciks":["0001373715"]}\n',
                encoding="utf-8",
            )

            sanitized = self.pack(source, root / "sanitized")
            extracted = root / "extracted-sanitized"
            data_snapshot.verify_snapshot(
                archive_path=Path(sanitized["archive_path"]),
                manifest_path=Path(sanitized["manifest_path"]),
                max_archive_bytes=1_000_000,
                extract_root=extracted,
            )

            self.assertEqual(clean["dataset_id"], sanitized["dataset_id"])
            self.assertFalse((extracted / "data/insiders").exists())
