...
class PipelinePositionIdentityTests(unittest.TestCase):
    ...
    def test_atomic_json_write_rejects_foreign_temp_replacement_before_publish(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            path = root / "authoritative.json"
            temporary = root / ".authoritative.json.owned.tmp"
            original = b'{"generation":"current"}'
            ...
            def replace_temp_after_file_fsync(descriptor: int) -> None:
                nonlocal replaced
            ...
            with (
                mock.patch.object(pipeline.secrets, "token_hex", return_value="owned"),
                mock.patch.object(
                    pipeline.os,
                    "fsync",
                    side_effect=replace_temp_after_file_fsync,
                ),
                self.assertRaises(pipeline.FundDataError),
            ):
                pipeline._atomic_write_json(path, {"generation": "next"})

            self.assertTrue(replaced)
            self.assertEqual(original, path.read_bytes())
            self.assertEqual(foreign, temporary.read_bytes())

    def test_atomic_json_write_keeps_foreign_temp_after_base_exception(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            path = root / "authoritative.json"
            temporary = root / ".authoritative.json.owned.tmp"
            ...
            with (
                ...
                mock.patch.object(
                    pipeline.json,
                    "dump",
                    side_effect=replace_temp_then_interrupt,
                ),
    ...
    def test_atomic_json_write_removes_owned_temp_after_base_exception(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            path = root / "authoritative.json"
            temporary = root / ".authoritative.json.owned.tmp"
    ...
    def test_atomic_json_write_durably_admits_every_created_parent_component(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            path = root / "level-one" / "level-two" / "level-three" / "output.json"
            real_mkdir = pipeline.os.mkdir
            real_open = pipeline.os.open
            real_fstat = pipeline.os.fstat
            real_fsync = pipeline.os.fsync
            real_fchmod = pipeline.os.fchmod
            real_close = pipeline.os.close
            ...
            def tracked_mkdir(component, mode=0o777, *, dir_fd=None):
                parent_identity = identity(dir_fd) if dir_fd is not None else None
                result = real_mkdir(component, mode, dir_fd=dir_fd)
                events.append(
                    {
                        "operation": "mkdir",
            ...
            def tracked_open(candidate, flags, mode=0o777, *, dir_fd=None):
                fd = real_open(candidate, flags, mode, dir_fd=dir_fd)
                metadata = real_fstat(fd)
                fd_identity = metadata.st_dev, metadata.st_ino
                parent_identity = identity(dir_fd) if dir_fd is not None else None
                ...
                if is_directory:
                    labels.setdefault(fd_identity, os.fspath(candidate))
                else:
                    labels.setdefault(fd_identity, "temporary output file")
                return fd
            ...
            def tracked_fsync(fd):
                events.append({"operation": "fsync", "identity": identity(fd)})
                return real_fsync(fd)

            def tracked_fchmod(fd, mode):
                events.append(
                    {"operation": "fchmod", "identity": identity(fd), "mode": mode}
                )
                return real_fchmod(fd, mode)

            def tracked_close(fd):
                events.append({"operation": "close", "identity": identity(fd)})
            ...
            self.assertEqual({"generation": "next"}, json.loads(path.read_text()))
            created = [event for event in events if event["operation"] == "mkdir"]
            self.assertEqual(["level-one", "level-two", "level-three"], [
                event["component"] for event in created
            ])
            labels[created[0]["parent"]] = "initial parent"
            created_children: list[tuple[dict[str, object], dict[str, object]]] = []
            for created_event in created:
                created_index = events.index(created_event)
                child_open = next(
                    event
                    for event in events[created_index + 1 :]
                    if event["operation"] == "open"
                    and event["candidate"] == created_event["component"]
                    and event["parent"] == created_event["parent"]
                    and event["is_directory"]
                )
                labels[child_open["identity"]] = str(created_event["component"])
                created_children.append((created_event, child_open))

            durable_components: list[str] = []
            for created_event, child_open in created_children:
                child_open_index = events.index(child_open)
                parent_close_index = next(
                    index
                    for index, event in enumerate(events[child_open_index + 1 :], child_open_index + 1)
                    if event["operation"] == "close"
                    and event["identity"] == created_event["parent"]
                )
                child_fsync_index = next(
                    (
                        index
                        for index, event in enumerate(
                            events[child_open_index + 1 : parent_close_index],
                            child_open_index + 1,
                        )
                        if event["operation"] == "fsync"
                        and event["identity"] == child_open["identity"]
                    ),
                    None,
                )
                ...
                if (
                    child_fsync_index is not None
                    and parent_fsync_index is not None
                    and child_open_index < child_fsync_index < parent_fsync_index
                ):
                    durable_components.append(str(created_event["component"]))

            fsync_labels = [
                labels.get(event["identity"], "unknown")
                for event in events
                if event["operation"] == "fsync"
            ]
            self.assertEqual(
                ...
                durable_components,
                f"durable components={durable_components}; fsync sequence={fsync_labels}",
            )
            self.assertEqual(
                ["level-one", "level-two", "level-three"],
                [
                    labels.get(event["identity"])
                    for event in events
                    if event["operation"] == "fchmod" and event["mode"] == 0o777
                ],
            )

    def test_atomic_json_write_ignores_preplanted_predictable_temp_symlink(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            ...
            victim.write_text('{"generation":"victim"}', encoding="utf-8")
            predictable_temp = path.with_suffix(path.suffix + f".tmp.{os.getpid()}")
            predictable_temp.symlink_to(victim)

            pipeline._atomic_write_json(path, {"generation": "next"})

            self.assertEqual('{"generation":"victim"}', victim.read_text())
            self.assertFalse(path.is_symlink())
            self.assertEqual({"generation": "next"}, json.loads(path.read_text()))
            self.assertTrue(predictable_temp.is_symlink())

    def test_atomic_json_write_rejects_nonregular_final_targets(self) -> None:
    ...
    def test_atomic_json_write_rejects_replaced_existing_parent_before_open(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            parent = root / "intended"
            parent.mkdir()
            path = parent / "authoritative.json"
            path.write_text('{"generation":"current"}', encoding="utf-8")
            (parent / "original-sentinel.txt").write_text("original", encoding="utf-8")
            replacement = root / "replacement"
            replacement.mkdir()
            (replacement / "replacement-sentinel.txt").write_text(
            ...
            def replace_parent_before_open(candidate, *args, **kwargs):
                nonlocal replaced
                if not replaced and os.fspath(candidate) in {
                    os.fspath(parent),
                    os.fspath(canonical_parent),
                }:
                    replaced = True
            ...
            with (
                mock.patch.object(pipeline.os, "open", side_effect=replace_parent_before_open),
                self.assertRaises(pipeline.FundDataError),
            ):
                pipeline._atomic_write_json(path, {"generation": "next"})

            self.assertTrue(replaced)
            self.assertEqual("replacement", (parent / "replacement-sentinel.txt").read_text())
            self.assertFalse((parent / "authoritative.json").exists())
            self.assertEqual(
                '{"generation":"current"}',
                (preserved / "authoritative.json").read_text(),
            )
            self.assertEqual("original", (preserved / "original-sentinel.txt").read_text())

    def test_atomic_json_write_rejects_retargeted_ancestor_symlink_before_open(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            original_ancestor = root / "original-ancestor"
            replacement_ancestor = root / "replacement-ancestor"
            original_ancestor.mkdir()
            replacement_ancestor.mkdir()
            raw_ancestor = root / "via"
            raw_ancestor.symlink_to(original_ancestor, target_is_directory=True)
            raw_parent = raw_ancestor / "intended"
            canonical_parent = (original_ancestor / "intended").resolve()
            raw_parent.mkdir()
            path = raw_parent / "authoritative.json"
            path.write_text('{"generation":"current"}', encoding="utf-8")
            (canonical_parent / "original-sentinel.txt").write_text("original", encoding="utf-8")
            replacement_parent = replacement_ancestor / "intended"
            ...
            def retarget_ancestor_before_open(candidate, *args, **kwargs):
                nonlocal retargeted
                if not retargeted and os.fspath(candidate) in {
                    os.fspath(raw_parent),
                    os.fspath(canonical_parent),
                }:
                    retargeted = True
                    raw_ancestor.unlink()
                    raw_ancestor.symlink_to(replacement_ancestor, target_is_directory=True)
                return real_open(candidate, *args, **kwargs)
            ...
            with (
                mock.patch.object(
                    pipeline.os,
                    "open",
                    side_effect=retarget_ancestor_before_open,
                ),
                self.assertRaises(pipeline.FundDataError),
            ):
                pipeline._atomic_write_json(path, {"generation": "next"})
            ...
            self.assertEqual(
                "replacement",
                (replacement_parent / "replacement-sentinel.txt").read_text(),
            )
            self.assertFalse((replacement_parent / "authoritative.json").exists())
            self.assertEqual(
                '{"generation":"current"}',
                (canonical_parent / "authoritative.json").read_text(),
            )
            self.assertEqual("original", (canonical_parent / "original-sentinel.txt").read_text())

    def test_atomic_json_write_anchors_missing_parent_before_ancestor_retarget(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            original_ancestor = root / "original-ancestor"
            replacement_ancestor = root / "replacement-ancestor"
            ...
            raw_ancestor = root / "via"
            raw_ancestor.symlink_to(original_ancestor, target_is_directory=True)
            raw_missing = raw_ancestor / "missing"
            canonical_missing = (original_ancestor / "missing").resolve()
            path = raw_missing / "authoritative.json"
            (original_ancestor / "original-sentinel.txt").write_text(
                "original", encoding="utf-8"
            )
            (replacement_ancestor / "replacement-sentinel.txt").write_text(
                "replacement", encoding="utf-8"
            )
            real_mkdir = pipeline.os.mkdir
            retargeted = False

            def retarget_ancestor_before_creation(candidate, mode=0o777, *, dir_fd=None):
                nonlocal retargeted
                if not retargeted and (
                    os.fspath(candidate) == os.fspath(raw_missing)
                    or (candidate == "missing" and dir_fd is not None)
                ):
                    retargeted = True
                    raw_ancestor.unlink()
                    raw_ancestor.symlink_to(replacement_ancestor, target_is_directory=True)
                return real_mkdir(candidate, mode, dir_fd=dir_fd)

            with (
                mock.patch.object(
                    pipeline.os,
                    "mkdir",
                    side_effect=retarget_ancestor_before_creation,
                ),
                self.assertRaises(pipeline.FundDataError),
            ):
                pipeline._atomic_write_json(path, {"generation": "next"})
            ...
            self.assertTrue(retargeted)
            self.assertTrue(canonical_missing.is_dir())
            self.assertFalse((canonical_missing / "authoritative.json").exists())
            self.assertFalse((replacement_ancestor / "missing").exists())
            self.assertEqual(
                "replacement",
                (replacement_ancestor / "replacement-sentinel.txt").read_text(),
            )
            self.assertEqual("original", (original_ancestor / "original-sentinel.txt").read_text())

    def test_atomic_json_write_serializes_conflicting_writers_until_return(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            path = Path(tmpdir) / "authoritative.json"
            ...
            calls = 0
            outcomes: list[tuple[dict[str, str], dict[str, str]]] = []
            failures: list[BaseException] = []
            real_dump = pipeline.json.dump

            def synchronized_dump(payload, handle, *args, **kwargs):
                nonlocal active_writers, maximum_active_writers, calls
                with state_lock:
                    calls += 1
                    call_number = calls
                    active_writers += 1
            ...
            def write(payload: dict[str, str]) -> None:
                try:
                    pipeline._atomic_write_json(path, payload)
                    outcomes.append((payload, json.loads(path.read_text(encoding="utf-8"))))
                except BaseException as exc:  # noqa: BLE001 - asserted in parent thread
                    failures.append(exc)

            with mock.patch.object(pipeline.json, "dump", side_effect=synchronized_dump):
                first = threading.Thread(target=write, args=({"writer": "first"},))
                second = threading.Thread(target=write, args=({"writer": "second"},))
                first.start()
            ...
            self.assertEqual([], failures)
            self.assertEqual(1, maximum_active_writers)
            self.assertCountEqual(
                [({"writer": "first"}, {"writer": "first"}), ({"writer": "second"}, {"writer": "second"})],
                outcomes,
            )
                ...
                with self.subTest(failure_type=failure_type.__name__):
                    ...
                    def fail_during_publish(
                        ...
                        **kwargs,
                        ...
                        source_is_stage = source_path.parent.name.startswith(
                            ".derived-stage-"
                        )
                        ...
                        if source_fd is not None:
                            ...
                            source_is_stage = any(
                                source_inode == stage.stat().st_ino
                                for stage in Path(tmpdir).glob(".derived-stage-*")
                            )
                        if target_path.name == "funds-index.json" and source_is_stage:
